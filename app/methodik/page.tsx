...
export const metadata = createPageMetadata({
  title: "Methodik",
  description: "So berechnet PassendPlanen Bedarf, prüft harte Kriterien, dokumentiert Quellen und begründet Produktempfehlungen.",
  path: "/methodik/",
});
...
export default function MethodikPage() {
  return (
    <article className="method-page">
      ...
      <header><p className="eyebrow">Nachvollziehbar statt scheinpräzise</p><h1>So arbeitet PassendPlanen.</h1><p>Unsere Werkzeuge trennen Bedarfsermittlung, harte Eignungskriterien und erklärende Sortierung. Der Rechenweg bleibt deterministisch: gleiche Eingaben führen zu denselben Ergebnissen.</p></header>
      ...
      <section className="method-detail"><div><p className="eyebrow">Redaktionsstandard</p><h2>Quellen erklären den Rahmen, nicht eine erfundene Gewissheit.</h2></div><div className="method-cards">
        <article><h3>Mit Primärquellen arbeiten</h3><p>Amtliche Rechtsquellen, Behörden, technische Systemunterlagen und Fachverbände haben Vorrang vor zusammenfassenden Ratgebertexten. Jede verlinkte Quelle erhält einen kurzen Hinweis, welche Aussage sie stützt.</p></article>
        <article><h3>Heuristik bleibt Heuristik</h3><p>Eigene Reserven, Klassengrenzen und Mengenschätzungen werden als PassendPlanen-Heuristik bezeichnet. Sie werden nicht als Norm, Herstellerfreigabe oder allgemeingültige Fachregel ausgegeben.</p></article>
      ...
      <section className="method-detail"><div><p className="eyebrow">Aktualität</p><h2>Prüfdatum und Korrekturweg gehören zum Inhalt.</h2></div><div><p>Ratgeber zeigen ein sichtbares und maschinenlesbares Prüfdatum. Quellen und Berechnungen werden bei fachlichen Änderungen aktualisiert; eine reine Layoutänderung erhält kein künstlich neues Inhaltsdatum.</p><p>Hinweise auf mögliche fachliche oder redaktionelle Fehler können über den im Impressum genannten Kontakt gemeldet werden. Inhaltliche Korrekturen erhalten ein neues Prüfdatum, reine Layoutänderungen nicht.</p><p><Link className="text-link" href="/garten/gartenhaus-groesse/">Beispiel mit Quellen und Rechenweg ansehen →</Link></p></div></section>
      <section className="method-detail"><div><p className="eyebrow">Maschinenlesbare Veröffentlichung</p><h2>Wichtige Inhalte sind auch für Suchsysteme klar auffindbar.</h2></div><div><p>Datenauswertungen enthalten strukturierte Angaben zu Thema, Stichprobe, Messgrößen, Autor und Aktualität. Der öffentliche Datenkatalog führt alle Auswertungen an einer Stelle zusammen. Die zugrunde liegenden Rohkataloge bleiben von der Indexierung ausgeschlossen.</p><p><Link href="/llms.txt">llms.txt</Link> bietet eine kurze, kuratierte Übersicht. <Link href="/llms-full.txt">llms-full.txt</Link> ergänzt den fachlichen Rahmen und alle öffentlichen Datenseiten. Beide Dateien erleichtern die Orientierung, sind aber weder eine Rankingzusage noch eine Zugriffserlaubnis für Crawler.</p><p><Link className="text-link" href="/ratgeber/daten/">Öffentlichen Datenkatalog ansehen →</Link></p></div></section>
      <section className="method-limit"><h2>Was PassendPlanen nicht verspricht</h2><p>Die Ergebnisse sind Planungshilfen und keine Genehmigung, Fachplanung, bauphysikalische Prüfung, Gesundheitsdiagnose oder Sicherheitsprüfung. Örtliche Bedingungen, aktuelle Herstellerangaben und vollständig geprüfte Systemaufbauten haben Vorrang. Mehr dazu steht in den <Link href="/nutzungshinweise/">Nutzungshinweisen und Haftungsregeln</Link>.</p></section>
    </article>
  );
}
