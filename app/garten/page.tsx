...
export default function GardenPage() {
  ...
  return <><section className="page-hero"><Breadcrumbs items={[{ label: "Start", href: "/" }, { label: "Garten" }]} /><p className="eyebrow">Projektbereich Garten</p><h1>Mehr Klarheit zwischen Idee und Aufbau.</h1><p>Werkzeuge und Ratgeber, die Platzbedarf, technische Grenzen und Gesamtkosten zusammenbringen.</p></section><section className="section"><Link className="planner-banner" href="/garten/gartenhaus-planer/"><div><span className="status-pill">Planer</span><h2>Welches Gartenhaus passt zu deinem Projekt?</h2><p>In wenigen Schritten zu Mindestfläche, Türbreite und kompatiblen Modellen.</p></div><span className="button button--light">Jetzt berechnen →</span></Link><Link className="planner-banner planner-banner--secondary" href="/garten/bewaesserungs-planer/"><div><span className="status-pill">Neu</span><h2>Bewässerung als kompatibles System planen.</h2><p>Materialstruktur für Rasen, Beete und Hecken – ohne falschen Hydraulikanspruch.</p></div><span className="button button--light">System planen →</span></Link><div className="guide-grid">{guides.map(([title, text, href]) => <Link href={href} className="guide-card" key={href}><span className="guide-number">MachPlan Wissen</span><h2>{title}</h2><p>{text}</p><span className="card-link">Ratgeber lesen →</span></Link>)}</div></section></>;
}
