...
export function DrywallPlanner() {
  ...
  const { value: input, setValue: setInput, reset: resetInput } = usePlannerSessionState("passendplanen:drywall:v1", INITIAL, parseInput);
  ...
  return <CalculatorShell planner="drywall" step={step} totalSteps={4} title={TITLES[step - 1]} label="Trockenbauwand-Rechner" onReset={() => { resetInput(); setStep(1); resetValidation(); }}>
    ...
    {step === 4 && plan && <div className="results drywall-results" aria-live="polite">
      ...
      <ProjectProductRecommendations requirements={{ vertical: "drywall", requiredKinds: ["board", "profile", "fastening", "joint", ...(input.includeInsulation ? ["insulation" as const] : []), "sealing"], purchaseAreaM2: plan.purchaseAreaM2, boardLengthMm: input.boardLengthM * 1000, boardWidthMm: input.boardWidthM * 1000, boardCount: plan.boardCount, profileLengthMm: input.trackBarLengthM * 1000, trackBarCount: plan.trackBarCount, moistureRequired: input.moistureExposure, fireOrAcousticRequired: input.fireOrAcousticRequirement, insulationRequired: input.includeInsulation }} />
