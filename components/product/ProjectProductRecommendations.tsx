...
export function ProjectProductRecommendations({ requirements }: { requirements: ProjectRequirements }) {
  ...
  useProductResultTracking({ planner: requirements.vertical, ready: catalog !== null, matchCount: matches.length, technicalMatchCount: technicalMatches.length });
  return <section className="recommendation-section"><p className="eyebrow">Geprüfte Angebote</p><h3>{technicalMatches.length ? "Technisch passende Produkte" : "Geprüfte Kategorieangebote"}</h3><p>Die Auswahl nutzt die Maße und Mengen aus deiner Berechnung. Wo technische Produktdaten fehlen, wird das Angebot ausdrücklich nur als Kategorieprodukt gezeigt.</p><AffiliateDisclosure /><CatalogMatchList planner={requirements.vertical} matches={technicalMatches} emptyLabel="Für diese Berechnung ist derzeit kein Angebot mit ausreichend bestätigten technischen Daten verfügbar." />{relatedMatches.length > 0 && <div className="recommendation-section__related"><h3>Alternativen und mögliche Ergänzungen</h3><p>Diese Angebote gehören zum Projektbereich, sind aber nicht als technischer Match bestätigt.</p><CatalogMatchList planner={requirements.vertical} matches={relatedMatches} emptyLabel="Keine weiteren geprüften Kategorieangebote verfügbar." /></div>}</section>;
}
