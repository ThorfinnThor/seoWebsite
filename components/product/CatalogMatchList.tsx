...
export function CatalogMatchList({ matches, emptyLabel }: { matches: Match[]; emptyLabel: string }) {
  ...
  return <div className="product-list" aria-label="Passende geprüfte Angebote">{matches.map((match, index) => <article className="product-card" key={match.product.id}>
    ...
    <div className="product-content">
      ...
      <div className="offer-row"><div><p className="merchant">Angebot von {match.offer.merchantName}</p><PriceDisplay offer={match.offer} /></div><AffiliateLink className="button button--primary" href={match.offer.affiliateUrl} productId={match.product.id}>Beim Händler ansehen <span aria-hidden="true">↗</span></AffiliateLink></div>
